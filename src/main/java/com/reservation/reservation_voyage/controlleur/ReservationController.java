...
public class ReservationController {
    ...
    @GetMapping(path = "/dayStatistique/{idUtilisateur}", produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseStatus(HttpStatus.OK)
    public ApiError getDayStatistique(@PathVariable UUID idUtilisateur){
        ApiError apiError = new ApiError();
        this.reservationService.getDayStatistique(idUtilisateur, apiError);
        return apiError;
    }

    @GetMapping(path = "/monthStatistique/{idUtilisateur}", produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseStatus(HttpStatus.OK)
    public ApiError getMonthStatistique(@PathVariable UUID idUtilisateur){
        ApiError apiError = new ApiError();
        this.reservationService.getMonthStatistique(idUtilisateur, apiError);
        return apiError;
    }

    @GetMapping(path = "/yearStatistique/{idUtilisateur}", produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseStatus(HttpStatus.OK)
    public ApiError getYearStatistique(@PathVariable UUID idUtilisateur){
        ApiError apiError = new ApiError();
        this.reservationService.getYearStatistique(idUtilisateur, apiError);
        return apiError;
    }

    @PutMapping(consumes = MediaType.APPLICATION_JSON_VALUE)
    @ResponseStatus(HttpStatus.ACCEPTED)
    public ApiError modifierReservation(@RequestBody Reservation reservation){
        ApiError apiError = new ApiError();
        this.reservationService.modifierReservation(reservation, apiError);
        return apiError;
    }
